<footer class="footer items-center bg-black px-4 py-10 text-white">
    <aside class="grid-flow-col items-center">
        <img src="svgs/tessellate_logo_white.svg" alt="Tessellate Logo" style="width: 40px" />
        <p class="font-display text-xl">TESSELLATE</p>
    </aside>
    <nav class="flex flex-col sm:flex-row sm:gap-8 md:place-self-center md:justify-self-end">
        <a
            href="https://www.instagram.com/tessellate_cmi"
            class="text-xl font-bold hover:underline hover:underline-offset-2"
        >
            Instagram &#129125;</a
        >
        <a
            href="https://www.facebook.com/CMITessellate"
            class="text-xl font-bold hover:underline hover:underline-offset-2"
        >
            Facebook &#129125;</a
        >
        <a
            href="https://twitter.com/cmitessellate"
            class="text-xl font-bold hover:underline hover:underline-offset-2"
        >
            Twitter &#129125;</a
        >
        <a
            href="https://www.youtube.com/TessellateCMI"
            class="text-xl font-bold hover:underline hover:underline-offset-2"
        >
            YouTube &#129125;</a
        >
    </nav>
    <!-- <a href="." class="text-slate-400"
        >Subscribe to the Tessellate newsletter to keep up with updates</a
    > -->
</footer>
